...
enum Info {
    ...
    struct Battery {
        /// false on a Mac without an internal pack — this is what keeps the
        /// battery surfaces hidden instead of showing empty readings
        var isPresent = false
        ...
        /// H:MM, matching the legacy menu-bar format. The I/O Kit sentinel
        /// 65535 means "still calculating", and a battery-less Mac has no
        /// estimate at all.
        var timeRemainingFormatted: String {
            let minutes = isCharging ? timeToFullCharge : timeToEmpty
            guard minutes > 0, minutes < 0xFFFF else {
                return "∞"
            }
        ...
        init() {
            guard
                ...
                let list = IOPSCopyPowerSourcesList(blob)?.takeRetainedValue() as? [AnyObject]
            else {
                return
            }

            // The list is unordered and also carries UPS/AC entries, so select
            // the built-in pack explicitly. IOPSGetPowerSourceDescription is the
            // documented accessor for a list element.
            var fallback: NSDictionary?
            var battery: NSDictionary?
            for source in list {
                guard
                ...
                if fallback == nil, description[kIOPSMaxCapacityKey] != nil {
                    fallback = description
                }
            }

            guard let dict = battery ?? fallback else {
                return
            }
