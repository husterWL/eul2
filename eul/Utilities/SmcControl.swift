...
class SmcControl: Refreshable {
    ...
    init() {
        ...
        do {
            ...
            print("SMC connection error", error)
            return
        }
        #if arch(x86_64)
            do {
                ...
                print("SMC temperature discovery error", error)
            }
        #endif
        do {
            guard try SMCKit.isKeyFound(FourCharCode(fromStaticString: "FNum")) else {
                return
            }
            ...
            print("SMC fan discovery error", error)
        }
    }
